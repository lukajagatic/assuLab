...
public class LabNet2 implements NeuralNetListener {
	...
	public LabNet2(int inputNeurons, int hiddenNeurons, int outputNeurons, String patternsFile, String outputFile) {
		...
		this.input.setRows(outputNeurons);
		this.output.setRows(inputNeurons);
	...
	public void trainNetwork(String patternsFile, String outputFile, double learningRate, double moment, int cycles,
		...
		String a = "";
		for (int i = inputNeurons; i < (outputNeurons + inputNeurons); i++) {
			a += i + 1;
		...
		String b = "";
		for (int i = 1; i < inputNeurons + 1; i++) {
			b += i;
			if (i != inputNeurons)
				b += ",";
		}
		...
		this.monitor.setTrainingPatterns(
				numberOfPatterns); /*
		...
		this.nnet.go(
				true); /*
						 * The net starts the training job, true - synchronized
						 */
	}

	public float[] getNetworkOutput(float[] input_parameters) {
		Layer input = this.nnet.getInputLayer();
		input.removeAllInputs();
		MemoryInputSynapse memInp = new MemoryInputSynapse();
		memInp.setFirstRow(1);
		memInp.setAdvancedColumnSelector("1-" + outputNeurons);
		input.addInputSynapse(memInp);
		double[][] codedPattern = new double[1][outputNeurons];
		...
		float[] resultPattern = new float[inputNeurons];
	...
	public float[] trainAndTest(String patternsFile, String outputFile, double learningRate, double moment,
			int cycles, int numberOfPatterns, float[] input_parameters) {
		this.trainNetwork(patternsFile, outputFile, learningRate, moment, cycles, numberOfPatterns);
		return this.getNetworkOutput(input_parameters);
	}

	public float[] trainAndTestSecond(String patternsFile, String outputFile, double learningRate, double moment,
			int cycles, int numberOfPatterns, float[] input_parameters) {
		this.trainNetwork(patternsFile, outputFile, learningRate, moment, cycles, numberOfPatterns);
		return this.getNetworkOutput(input_parameters);
	}
